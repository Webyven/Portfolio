...
const translations: Record<Language, Record<string, string>> = {
  en: {
    ...
    'projects.autocompleteShortcuts.longDescription':
      'Autocomplete Shortcuts is a Windows app that lets you create keyboard shortcuts to quickly paste text in any input field. Each shortcut has a trigger word and a value. When the trigger is typed, the app deletes it and pastes the corresponding value into the focused input. If the value contains "->", the app simulates a "tab" key press to move to the next input and continues pasting. It’s especially useful for filling login forms or entering repetitive data across apps. Users can add, remove, or copy shortcuts with ease.', // Descripción larga del proyecto
    ...
    'projects.filenameChanger.longDescription':
      'Filename Changer is a simple bulk renamer tool for Windows. It allows selecting a folder and replacing parts of filenames based on two text inputs: the string to find and the one to replace it with. It includes two additional options to trim the beginning and/or end of filenames. While it doesn’t support regex or advanced patterns, its minimal interface makes it fast and effective for everyday file renaming tasks.', // Descripción larga del proyecto
    ...
    'projects.copaPiston.longDescription':
      'Copa Pistón is a Windows desktop app designed to track kart racing tournaments with friends. Users can register the top finishers of each race, log fastest laps, attach files per race, and manage all data through a cloud-hosted SQL database. It also calculates championship points and generates a leaderboard. What started as a fun way to bring structure to friendly races turned into a robust system tailored for real events.', // Descripción larga del proyecto
    ...
    'projects.googleClone.longDescription':
      "Google Clone is a static replica of the original Google homepage built with HTML, CSS, and vanilla JavaScript. The search bar mimics Google's look and redirects the user to the actual search results on Google with the entered query. It was built as a layout and styling exercise and demonstrates pixel-accurate implementation with minimalist JS logic.", // Descripción larga del proyecto
